...
Файл [api.py](src\api.py) содержит в себе функции взаимодействия с базой данных. Основной библиотекой для взаимодействия является библиотека **sqlite3**.
...
В случае если пользователь уже записан в базе данных то запрос в базу данных вернет кортеж, длинна которого больше чем 0 и функция вернёт значение **True**. В противном случае функция возвращает **False**, обозначаем этим отсутствие в базе данных пользователя с указанным ID.
...
Файл принимает информацию о пользователя от [telegram.py](src\telegram.py) и вносит информацию в соответсвующее поле в таблицу **user** в базе данных.
...
def role_check(user_id):
    ...
    return role[0][0]
```
Функция принимает переменную ID пользователя и отправляет в базу данных запрос с поиском по указанному ID. Результат запроса содержит кортеж с кортежем. Функция возвращает значение столбца *role* из таблицы **users**, обращаясь по индексу первого значения первого элемента полученного от запроса результата. 
...
def book_existence_check(book):
    ...
    if len(res.fetchall()) == 0:
        ...
        return True
```
Функция принимает словарь с информацией о книге и путём исполнения запроса проверяет наличие указанной книги в базе данных. В случае если книга уже записан в базе данных то запрос в базу данных вернет кортеж, длинна которого больше чем 0 и функция вернёт значение **True**. В противном случае функция возвращает **False**, обозначаем этим отсутствие в базе данных искомой книги.
...
def book_id_serch(book_id):
    ...
    return res.fetchall()
```
Функция принимает ID книги, информацию о которой необходимо найти. Функция возвращает название, автора и дату год публикации из таблицы **books** в виде кортежа с кортежем.
...
def take_id(book):
    ...
    return id[0]
```
Функция принимает ID книги, которую нужно отправить. Функция возвращает значения столбца *path* из таблицы **books**, которое представляет собой путь до искомой книги.
...
Функция принимает словарь с информацией о книге, которую нужно добавить. Для того чтобы избежать повторения книга в базе данных функция обращается к другой функции - *book_existence_check*. В случае если книга не был до этого занесена в таблице **books** то произойдёт ее запись и функция вернёт значение **True**, в противном случает функция, возвращает значение **False**.  
...
def list_of_books():
    ...
    return res.fetchall()
```
Функция возвращает кортеж с кортежами, содержащими информацию о всех книгах записанных в таблице **books**.
